...
for i = 1:nfiles
    ...
    [pathname,~,~] = fileparts(which(list_files_mot{i})); % determine the name of the trial
    ...
    if isfield(raw_qtm_export.(root{1}).Trajectories.Labeled,'Labels')
        for j= 1:size(raw_qtm_export.(root{1}).Trajectories.Labeled.Labels,2);
            ...
            for k = 1:3
                m_filt(k,:) = LowPassButterworth(m_temp(k,:),4,8,marker_data.FrameRate);
            end
    ...
    if isfield(raw_qtm_export.(root{1}),'Force')
        ...
        for j = 1:numFP
            ...
            if ~isempty(force_data(j).Force)
                for k = 1:3
                    force_data(j).Force(k,:) = LowPassButterworth(force_data(j).Force(k,:),4,50,force_data(j).Frequency);
                end
            end
        end
    else
    ...
    clearvars -except list_files_mot nfiles varout savedir
end
